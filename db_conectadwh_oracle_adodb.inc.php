...
$dbdwh= NewADOConnection("oci8po"); 		//oracle 9.2 o superior
...
//						//	string de conexion, usuario, clave, base
 $dbdwh->Connect("(DESCRIPTION =
  ...
  (CONNECT_DATA =(SID = DWHOUSE)))","casino","pass123");
 
		// (CONNECT_DATA =(SID = CENTRAL)))","","esquema"); 
...
*/ 
	...
	die($dbdwh->ErrorMsg()."<br><br><a href=\"index.php\">Regresar a pagina anterior.</a><br><br>");
	}
...
function ComenzarTransaccion($dbdwh) {
	$dbdwh->StartTrans();
}

function FinalizarTransaccion($dbdwh) {
	$dbdwh->CompleteTrans($autoComplete=true);
}
